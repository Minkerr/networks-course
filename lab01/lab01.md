...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - [4 Reassembled TCP Segments (4864 bytes): #301(1348), #302(1348), #303(1348), #304(820)]
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   ...
   - у первых двух хост (Host: gaia.cs.umass.edu\r\n). у последнего (Host: kurose.cslash.net\r\n)
   - Destination Address: 172.24.30.35
   - Destination Address: 172.24.30.35
   - Destination Address: 2a02:6bf:8005:f00:1dd4:28f5:4ff7:1339
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Последовательно. Понять можно по времени отправки запроса и получении ответа
